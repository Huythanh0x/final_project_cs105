...
function animate() {
    ...
    if (animationType == 1) {
        ...
        let sin_theta = Math.sin(Date.now()*0.002)
        if (sin_theta > 0 && sin_theta < old_sin) {
            sign = 1
        } else if (sin_theta < 0 && sin_theta > old_sin) {
            sign = -1
        }
        mesh.position.x = sin_theta * 5
        mesh.position.z = sign * Math.sqrt(25-Math.pow(mesh.position.x,2))
        old_sin = sin_theta
   }
